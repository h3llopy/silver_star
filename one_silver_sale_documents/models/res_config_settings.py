...
class ResConfigSettings(models.TransientModel):
    ...
    documents_sale_settings = fields.Boolean(
        related='company_id.documents_sale_settings', readonly=False,
        string="Sale"
    )
    sale_folder = fields.Many2one(
        'documents.folder', related='company_id.sale_folder', readonly=False,
        string="sale default workspace"
    )
    sale_tags = fields.Many2many(
        'documents.tag', 'sale_tags_table', related='company_id.sale_tags',
        readonly=False, string="sale Tags"
    )
