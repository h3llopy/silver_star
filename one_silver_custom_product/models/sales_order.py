    ...
    def _get_customer_products(self):
        if self.order_id.partner_id.allow_custom_product:
            ...
            if self.order_id.based_sale_order or self.order_id.is_free_order :
                ...
                free_types_global_products = self.env['product.template'].search([('allow_free', '=', True), ('free_types_allowed', 'in', free_types_global.ids + self.order_id.free_types.ids)])
                customer_free_standalone_products = self.env['product.template'].search([('allow_free', '=', True), ('free_types_allowed', 'in', self.order_id.free_types.ids),('id', '=', customer_products + free_types_global_products.ids)])
                return {'domain': {
            ...
            if self.order_id.based_sale_order or self.order_id.is_free_order :
                ...
                return {'domain': {
                    'product_template_id': "[('qty_available','>',0),('id','in', %s),('sale_ok', '=', True), '|', ('company_id', '=', False), ('company_id', '=', company_id)]" % customer_free_standalone_products.ids,
                    'product_id': "[('qty_available','>',0),('id','in', %s),('sale_ok', '=', True), '|', ('company_id', '=', False), ('company_id', '=', company_id)]" % customer_free_standalone_products.product_variant_ids.ids,
                }}
        else:
            return {'domain':
                {
                    'product_template_id': "[('qty_available','>',0),('sale_ok', '=', True), '|', ('company_id', '=', False), ('company_id', '=', company_id)]",
                    'product_id': "[('qty_available','>',0),('sale_ok', '=', True), '|', ('company_id', '=', False), ('company_id', '=', company_id)]",
                }
            }