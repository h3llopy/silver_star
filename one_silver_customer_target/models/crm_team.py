...
class CrmTeam(models.Model):
    ...
    manager_id = fields.Many2one(comodel_name='hr.employee', compute='_compute_manger_id', store=True)
    @api.depends('user_id')
    def _compute_manger_id(self):
        """ Set Manager """
        for rec in self:
            if rec.user_id:
...
class CrmTeamMember(models.Model):
    ...
    def _user_id_domain(self):
        in_group_ids = self.env['res.users'].search(
            [('groups_id', 'in', self.env.ref('one_silver_customer_target.agent_user').id)])
        return {'domain': {'user_id': [('id', 'in', in_group_ids.ids), ('id', 'not in', self.user_in_teams_ids.ids), ('company_ids', 'in', self.user_company_ids.ids)]}}
